...
process STAR {
    ...
    output:
        tuple val(sample_id), path("${sample_id}_matrix.mtx"), path("${sample_id}_features.tsv"), path("${sample_id}_barcodes.tsv"), emit: mtx_files
...
process MTXTOHDF5 {
    ...
    input:
        tuple val(sample_id), path(mtx), path(features), path(barcodes)
